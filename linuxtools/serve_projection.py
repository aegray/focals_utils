...
async def publish_state(ws, n):
    #await ws.send(json.dumps({"type":"current_state","state":"currently_presenting","title":"stuff","notes":"<font size=23>去商店: %s<br><br>HELLO</font>"%(n),"slide_number":-1,"total_slides":5}))

    await ws.send(json.dumps({"type":"current_state","state":"currently_presenting","title":"stuff","notes":"val: %s"%(n),"slide_number":-1,"total_slides":5}))


async def serve_projection(ws, path):
    print("PATH=", path)
    await publish_state(ws, onind)
...
#        await ws.send(json.dumps({"type":"current_state","state":"currently_presenting","title":"stuff","notes":"<font size=23>去商店: 4<br><br>HELLO</font>","slide_number":-1,"total_slides":5}))
#
#                �{"type":"connected"}
...
server = websockets.serve(serve_projection, "0.0.0.0", 8001)
