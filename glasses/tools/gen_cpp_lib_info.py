...
v = subprocess.check_output("arm-linux-androideabi-nm --dynamic %s"%(sys.argv[1]), shell=True).split(b'\n')
for p in v:
    p = p.split()
    if p:
        if p[0] != b'U':
            if p[-1].startswith(b'_Z'):
                #print(p[-1], ' : ', demangle(p[-1].decode('ascii')))
                try:
                    print(demangle(p[-1].decode('ascii')))
                except ValueError:
                    print("Could not decode: ", p[-1])

#
#       •   The symbol type.  At least the following types are used; others are, as well, depending on the object file format.  If lowercase, the symbol is usually local; if uppercase, the symbol is global (external).  There are however a few lowercase symbols that
...
#
#           "i" For PE format files this indicates that the symbol is in a section specific to the implementation of DLLs.  For ELF format files this indicates that the symbol is an indirect function.  This is a GNU extension to the standard set of ELF symbol types.
#               It indicates a symbol which if referenced by a relocation does not evaluate to its address, but instead must be invoked at runtime.  The runtime execution will then return the value to be used in the relocation.
#
...
#               becomes zero with no error.  On some systems, uppercase indicates that a default value has been specified.
#
#           "W"
